...
if(additive_only){
  ...
  write.csv(grid, file = paste0("GP_modeling/grids/grid_keri_GEN",keri,".csv"), row.names = FALSE)
...
for(ss in 1:2){
  ...
  for(i in 1:nr_outer_split){
    ...
    logLH <- foreach(j = 1:nrow(grid), .combine = 'c',
      ...
      while(sum(c(res1,res2,res3,res4)%>%is.nan)>0||
        ...
        theta2 <- grid[mm,1]
        alpha <- grid[mm,2]
          initial_par=c(theta2,alpha)%>%unname
  ...
  if(GA_kernel){
       save(list = "Initial_res", 
           file = paste0('Results/Results_gen_only/Results grid search/Initial_par_',
