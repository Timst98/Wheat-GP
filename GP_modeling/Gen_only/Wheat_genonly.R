...
grid= as.matrix(read.csv(paste0("GP_modeling/grids/grid_keri_GEN",keri,".csv")))
  ...
  create_kernelmat_derivative <-
    function(kernel_meteo,
      ...
      if(kernel_geno=='dos'){
        ...
        Kmat_geno <- exp(-Dmat / (int))
        Kmat_geno_deriv <-
          Kmat_geno * Dmat / (int ^ 2) * max(D_gblup)
      }
      # spectrum kernel
      if (kernel_geno == 'spe') {
        s <- k
      ...
      Kmat=as.matrix(Kmat_geno)
      return(
        list(
          Kmat = Kmat,
          del_theta2 = as.matrix(Kmat_geno_deriv)
...
log_likelihood = function(theta2,
                          alpha,
                          train) {
...
likelihood_gradient = function(theta2,
                               alpha,
                               train) {
  Trace = function(x) {
    sum(diag(x))
    ...
    zn = train[[tar]]
      ...
      Kobs_inv_deriv = -(v * Kobs_inv) %*% as.matrix(alpha * kernel_matrices$del_theta2) %*%
        (v * Kobs_inv)
      ...
      del_v_theta2 = del_v_theta2 %>% as.numeric()
      
      
      Kobs_inv_deriv_theta2 = -Kobs_inv %*% as.matrix(v * alpha * kernel_matrices$del_theta2) %*%
        Kobs_inv +
        
        del_v_theta2 * v * Kobs_inv
...
Res = foreach(
  ...
  for (i in 1:nr_outer_split) {
    if(GA_kernel){
        ...
        load(paste0('Results/Results_gen_only/Results grid search/Initial_par_',
...
tic()
      ...
      time_inference=toc()
      time_total=time_optimization+time_inference
      ...
      if (NLOPTR) {
        if (validation_gradient) {
          ...
          save(
            list = "res",
            file = paste0(
              'Results/Results_gen_only/Results LBFGS/Results_hyper_VAL_',
              NLOPT_alg,
              '_',
              batchsize,
              '_',
          ...
          save(
            list = "res",
            file = paste0(
              'Results/Results_gen_only/Results LBFGS/Results_hyper_',
              NLOPT_alg,
              '_',
              batchsize,
              '_',
        ...
        if (validation_gradient) {
          save(
            list = "res",
            file = paste0(
              'Results/Results_gen_only/Results Adam/Results_hyper_VAL_ADAM_',
              lr,
              '_',
              max_iter,
              '_',
          ...
          save(
            list = "res",
            file = paste0(
              'Results/Results_gen_only/Results Adam/Results_hyper_ADAM_',
              lr,
              '_',
              max_iter,
              '_',
