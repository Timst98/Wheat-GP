...
leak_spec <- 'random'
additive_only=as.integer(Sys.getenv("ADDONLY"))
product_only=as.integer(Sys.getenv("PRODONLY"))
...
for(ss in 1:2){
  ...
  logLH <- foreach(j = 1:nrow(grid), .combine = 'c',
                       .export = c('grid', 'create_kernelmat_derivative', 'train', 'kernel_meteo', 
                                   'kernel_geno', 'product_only', 'additive_only', 'noisy_obs', 'tar'),
                       .packages = c("mvtnorm", "MASS", "R.utils", "doParallel", "foreach")) %dopar% {
    ...
    if(GA_kernel){
      Initial_res=list(ind_train,initial_par,-2*max(logLH),res1,res2,res3,res4,
                       logLH,kernel_matrices$Kernel_GA,
                       kernel_matrices$Kernel_GA_deriv)
    }else{
      Initial_res=list(ind_train,initial_par,-2*max(logLH),res1,res2,res3,res4,
                       logLH)
    }
  ...
  if(GA_kernel){
    if(additive_only){
      save(list = "Initial_res", 
           file = paste0('Results/Results_additive_only/Results grid search/Initial_par_',
                         tar, keri,'GA', leakage,setupp,'_',id,'.rda'))
      
    }else{
      if(product_only){
        save(list = "Initial_res", 
             file = paste0('Results/Results_product_only/Results grid search/Initial_par_',
                           tar, keri,'GA', leakage,setupp,'_',id,'.rda'))
      }else{
        save(list = "Initial_res", 
             file = paste0('Results/Results grid search/Initial_par_',
                           tar, keri,'GA', leakage,setupp,'_',id,'.rda'))
      }
    }
  }else{
